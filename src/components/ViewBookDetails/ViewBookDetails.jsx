...
const ViewBookDetails = () => {
  ...
  console.log(isLoggedIn, role);
    ...
    const handleFavourite = async() => {
      const response = await axios.put("https://bookista.vercel.app/api/v1/add-book-to-favourite",{},{headers});
      alert(response.data.message);
    };

    const handleCart = async() => {
      const response = await axios.put("https://bookista.vercel.app/api/v1/add-to-cart",{},{headers});
      alert(response.data.message);
    };
    const deleteBook = async() =>{
      const reponse =await axios.delete("https://bookista.vercel.app/api/v1/delete-book",{headers});
 ...
 {isLoggedIn === true && role === "admin" && (
  ...
  <Link to={`/updateBook/${id}`} className='text-black rounded lg:rounded-full text-3xl p-3 bg-white flex items-center justify-center'>
  <FaEdit />
