...
const Cart = () => {
  ...
  const deletItem = async (bookid) => {
    try {
      const response = await axios.put(`https://backend-api-2x5r.vercel.app/api/v1/remove-from-cart/${bookid}`, {}, { headers });
...
const PlaceOrder = async () => {
  try{
    const response = await axios.post("https://bookista.vercel.app/api/v1/place-order", 
      {order: Cart}, 
      { headers }
    );
    alert(response.data.message);
